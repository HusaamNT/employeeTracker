...
const employeeInfo = async function () {
  ...
  const listEmployee = arrayE.map((it) => ({
    value: it.id,
    name: it.first_name,
  }));
...
async function addDepartment() {
    const answers = await departmentInfo();
    console.log(answers);
    console.log(answers.departmentName)
    const [result] = await db.query(`INSERT INTO department (name) values (?)`, [answers.departmentName]);
    startQuestions();
}
...
async function addRole() {
  ...
  const [result] = await db.query(
    ...
    [answers.title, answers.salary, answers.roleDepartmentId]
  );
  startQuestions();
}
